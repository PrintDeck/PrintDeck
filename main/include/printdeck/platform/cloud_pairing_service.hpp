...
class CloudPairingService {
 public:
  ...
  void initialize();
  void bind_persistence(PersistenceWorker& worker);
 ...
 private:
  ...
  std::uint32_t screen_upload_seq_ = 0;
  ...
  std::uint32_t thumbnail_printer_ = 0, thumbnail_generation_ = 0;
  bool thumbnail_complete_ = false;
  unsigned thumbnail_deferred_ = 0;
  std::int64_t thumbnail_log_due_ = 0;
  void defer_thumbnail(const char* reason);
  std::int64_t thumbnail_due_ = 0;
  enum class SaveAction { disconnect, forget, account, authorize };
  struct CredentialWrite {
    std::string token, account, blob, storage, state;
    SaveAction action;
    std::uint32_t generation;
    std::int64_t deadline;
  };
  bool queue_save(SaveAction action, const std::string& token,
                  const std::string& account, const std::string& state = {});
  static std::uint32_t persist_entry(void* context);
  struct LocalWork {
  ...
  bool queue_local_work(std::unique_ptr<LocalWork> work);
  std::unique_ptr<LocalWork> local_work_;
  bool local_work_running_ = false;
  PersistenceWorker* persistence_ = nullptr;
  std::unique_ptr<CredentialWrite> credential_write_;
  ...
  bool download_reaction(const std::string& base, const std::string& token, bool local, const std::string& asset,
                         std::size_t size, const std::string& hash, const std::string& payload, std::uint32_t generation, const std::string& command_id);
