...
std::string moonraker_host_identity(const core::PrinterProfile& profile,
  ...
  if (cancelled() || response.error != PrusaLinkError::none || response.status != 200) return {};
  ...
  if (moonraker.empty() || klipper.empty() || moonraker == "unknown" || klipper == "unknown") return {};
  ...
  if (macs.empty() || macs.size() > 8) return {};
  ...
  return core::valid_moonraker_uuid(digest) ? "mr:" + digest : std::string{};
}
...
std::optional<core::PrinterProfile> learn_moonraker_identity(
  ...
  if (profile.protocol != core::PrinterProtocol::moonraker || !profile.network_identity.empty() || cancelled()) return {};
  const auto address = core::printer_address(profile);
  if (!address) return {};
  ...
  learned.network_identity = identity ? *identity : moonraker_host_identity(profile, cancelled);
...
bool moonraker_endpoint_identity_matches(const core::PrinterProfile& profile) {
  ...
  if (profile.network_identity.starts_with("mr:"))
    return moonraker_host_identity(profile, [] { return false; }) == profile.network_identity;
  const auto address = core::printer_address(profile);
  ...
  for (const auto& service : moonraker_services()) {
    ...
    if (service.uuid != profile.network_identity) return false;
    matched = true;
  }
  return matched;
}
