...
esp_err_t rotation_panel_draw(esp_lcd_panel_t* panel, int x_start, int y_start,
  ...
  if (state->software_rotation == 0) {
    // LVGL draws into PSRAM, which this SPI transport cannot send directly.
    // Use the startup-reserved DMA slot even without rotation: otherwise the
    // SPI driver allocates a temporary buffer for each update and can lose
    // complete display strips when the internal heap becomes fragmented.
