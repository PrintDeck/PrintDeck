...
PrusaLinkPollResult octoprint_probe(const core::PrinterProfile& profile,
  ...
  OctoPrintClient client(transport, prusalink_now_ms);
  if (profile.protocol != core::PrinterProtocol::octoprint ||
      !client.configure(profile.endpoint, profile.api_key, profile.id))
...
void OctoPrintAdapter::configure(const core::PrinterProfile* profile) {
  const std::lock_guard<std::mutex> lock(mutex_);
  const core::PrinterProfile next = profile && profile->protocol == core::PrinterProtocol::octoprint
...
esp_err_t OctoPrintAdapter::start(const core::PrinterProfile* profile, const NetworkService& network) {
  configure(profile);
  const std::lock_guard<std::mutex> lock(mutex_);
  if (running_) return stopping_ ? ESP_ERR_INVALID_STATE : ESP_OK;
  if (!profile_.id) return ESP_ERR_INVALID_ARG;
  // A reopened profile must earn online state from a fresh response, even when
  // its endpoint and credentials match the previous worker.
  snapshots_.invalidate(profile_.id, core::LinkState::connecting, prusalink_now_ms());
  network_ = &network;
  stopping_ = false;
  running_ = true;
  if (xTaskCreatePinnedToCoreWithCaps(task_entry, "octoprint", 24576, this, 4, &task_,
      kServiceCore, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT) != pdPASS) {
    running_ = false;
    task_ = nullptr;
    snapshots_.invalidate(profile_.id, core::LinkState::failed, prusalink_now_ms());
    return ESP_ERR_NO_MEM;
  }
  return ESP_OK;
}

void OctoPrintAdapter::stop() {
  const std::lock_guard<std::mutex> lock(mutex_);
  stopping_ = true;
  if (task_) xTaskNotifyGive(task_);
}

void OctoPrintAdapter::task_entry(void* context) {
  static_cast<OctoPrintAdapter*>(context)->run();
  vTaskDeleteWithCaps(nullptr);
}

void OctoPrintAdapter::run() {
  PrusaLinkEspTransport transport;
  OctoPrintClient client(transport, prusalink_now_ms);
  ...
  while (!stopping_) {
    core::PrinterProfile profile;
    {
      ...
      if (!configured || revision != generation_) {
        ...
        configured = client.configure(profile.endpoint, profile.api_key, profile.id);
      }
    }
    if (!profile.id) break;
    ...
    if (result.sample && !reported_stack) {
      ESP_LOGI("octoprint", "Status worker stack high-water=%u",
    ...
    // Bounded periodic reads belong only to the selected live printer.
    ulTaskNotifyTake(pdTRUE, pdMS_TO_TICKS(2000));
  }
  ESP_LOGI("octoprint", "Stopped worker stack high-water=%u",
           static_cast<unsigned>(uxTaskGetStackHighWaterMark(nullptr)));
  const std::lock_guard<std::mutex> lock(mutex_);
  task_ = nullptr;
  running_ = false;
...
esp_err_t OctoPrintConnectionProbe::start(core::PrinterProfile profile, const NetworkService& network) {
  const std::lock_guard<std::mutex> lock(mutex_);
  ...
  if (xTaskCreatePinnedToCoreWithCaps(task_entry, "octoprint_check", 24576, this, 4, nullptr,
      kServiceCore, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT) != pdPASS) {
...
void OctoPrintConnectionProbe::cancel(std::string_view id) {
  const std::lock_guard<std::mutex> lock(mutex_);
...
OctoPrintCheckSnapshot OctoPrintConnectionProbe::snapshot() const {
  const std::lock_guard<std::mutex> lock(mutex_);
  return snapshot_;
}

bool OctoPrintConnectionProbe::verified(const core::PrinterProfile& profile, std::string_view id) const {
  const std::lock_guard<std::mutex> lock(mutex_);
...
void OctoPrintConnectionProbe::task_entry(void* context) {
  static_cast<OctoPrintConnectionProbe*>(context)->run();
  vTaskDeleteWithCaps(nullptr);
}

void OctoPrintConnectionProbe::run() {
  core::PrinterProfile profile;
  {
    const std::lock_guard<std::mutex> lock(mutex_);
    profile = profile_;
  ...
  const auto result = octoprint_probe(profile, prusalink_now_ms() + 12000, cancelled, &identity);
  ESP_LOGI("octoprint", "Verification worker stack high-water=%u",
           static_cast<unsigned>(uxTaskGetStackHighWaterMark(nullptr)));
  const std::lock_guard<std::mutex> lock(mutex_);
  snapshot_.running = false;
