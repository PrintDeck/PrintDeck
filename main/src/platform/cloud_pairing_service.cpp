      ...
      if(reply.status==200 && cJSON_IsTrue(cJSON_GetObjectItemCaseSensitive(data,"accepted"))) {
        ...
        if(command_sink_ && cJSON_IsArray(commands) && cJSON_GetArraySize(commands)==1) {
          ...
          const bool valid=!id.empty() && id.size()<=20 && id.front()!='0' &&
              ...
              std::floor(ttl->valuedouble)==ttl->valuedouble && (core::parse_printer_command(payload,parsed) || (core::parse_device_command(payload,device_command) && ((core::device_restart_command(device_command) || device_command.action=="printers.reorder" || device_command.action=="device.printer_view.set" || device_command.action=="device.name.set" || device_command.action=="device.voice.set") || (device_command.action=="device.timezone.set" || device_command.action=="device.unified_api.set" || device_command.action=="device.mqtt.patch" || device_command.action=="device.appearance.patch" || device_command.action=="audio.test" || device_command.action=="firmware.check" || device_command.action=="firmware.install" || core::cloud_reaction_control(device_command.action) || device_command.action=="reactions.image.upload"))));
          if(valid) {
