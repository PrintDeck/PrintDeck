...
bool add_profiles(cJSON* settings_object, const std::vector<PrinterProfile>& profiles) {
  ...
  for (const PrinterProfile& profile : profiles) {
    ...
    if (!add_number(object, "id", profile.id) ||
        ...
        !add_string(object, "network_identity", profile.network_identity) ||
...
bool add_settings(cJSON* root, const DeviceSettings& settings) {
  ...
  return object && add_string(object, "device_name", settings.device_name) &&
         ...
         cJSON_AddArrayToObject(object, "companion_cameras") && // Empty legacy field for older backup readers.
