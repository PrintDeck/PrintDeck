...
const PRINTDECK_EXTRA_TRANSLATIONS={
  "Device restart":["Restart urządzenia", "Reinicio del dispositivo", "Redémarrage de l’appareil", "Gerät neu starten", "设备重启"],
  "Restart PrintDeck without changing saved settings. Your printer will keep printing.":["Uruchom ponownie PrintDeck bez zmiany zapisanych ustawień. Drukarka będzie kontynuować drukowanie.", "Reinicia PrintDeck sin cambiar los ajustes guardados. Tu impresora seguirá imprimiendo.", "Redémarrez PrintDeck sans modifier les paramètres enregistrés. Votre imprimante continuera à imprimer.", "Starte PrintDeck neu, ohne gespeicherte Einstellungen zu ändern. Dein Drucker druckt weiter.", "重启 PrintDeck，不更改已保存的设置。打印机将继续打印。"],
  "Restart device":["Uruchom ponownie", "Reiniciar dispositivo", "Redémarrer l’appareil", "Gerät neu starten", "重启设备"],
  "Restart PrintDeck?":["Uruchomić ponownie PrintDeck?", "¿Reiniciar PrintDeck?", "Redémarrer PrintDeck ?", "PrintDeck neu starten?", "重启 PrintDeck？"],
  "PrintDeck will disconnect briefly. Wait for it to reconnect before making more changes.":["PrintDeck na chwilę się rozłączy. Poczekaj na ponowne połączenie przed wprowadzeniem kolejnych zmian.", "PrintDeck se desconectará brevemente. Espera a que vuelva a conectarse antes de hacer más cambios.", "PrintDeck se déconnectera brièvement. Attendez sa reconnexion avant d’effectuer d’autres modifications.", "PrintDeck wird kurz getrennt. Warte auf die erneute Verbindung, bevor du weitere Änderungen vornimmst.", "PrintDeck 将短暂断开连接。请等待重新连接后再进行其他更改。"],
  "Restarting PrintDeck… Waiting for reconnection.":["Ponowne uruchamianie PrintDeck… Oczekiwanie na połączenie.", "Reiniciando PrintDeck… Esperando la reconexión.", "Redémarrage de PrintDeck… En attente de reconnexion.", "PrintDeck wird neu gestartet… Warte auf die Verbindung.", "正在重启 PrintDeck… 等待重新连接。"],
  "Restart requested. Waiting for PrintDeck…":["Wysłano żądanie restartu. Oczekiwanie na PrintDeck…", "Reinicio solicitado. Esperando a PrintDeck…", "Redémarrage demandé. En attente de PrintDeck…", "Neustart angefordert. Warte auf PrintDeck…", "已请求重启。正在等待 PrintDeck…"],
  "PrintDeck restarted and is connected again.":["PrintDeck uruchomił się ponownie i jest już połączony.", "PrintDeck se ha reiniciado y está conectado de nuevo.", "PrintDeck a redémarré et est de nouveau connecté.", "PrintDeck wurde neu gestartet und ist wieder verbunden.", "PrintDeck 已重启并重新连接。"],
  "Reconnection was not confirmed. Check the device connection before trying again.":["Nie potwierdzono ponownego połączenia. Sprawdź połączenie urządzenia przed kolejną próbą.", "No se ha confirmado la reconexión. Comprueba la conexión del dispositivo antes de intentarlo de nuevo.", "La reconnexion n’a pas été confirmée. Vérifiez la connexion de l’appareil avant de réessayer.", "Die erneute Verbindung wurde nicht bestätigt. Prüfe die Geräteverbindung, bevor du es erneut versuchst.", "未确认重新连接。再次尝试前请检查设备连接。"],
  "Restart is unavailable. Wait for PrintDeck to be online and finish its current operation.":["Restart jest niedostępny. Poczekaj, aż PrintDeck będzie online i zakończy bieżącą operację.", "El reinicio no está disponible. Espera a que PrintDeck esté conectado y termine la operación actual.", "Le redémarrage est indisponible. Attendez que PrintDeck soit connecté et termine l’opération en cours.", "Neustart nicht verfügbar. Warte, bis PrintDeck online ist und den aktuellen Vorgang beendet hat.", "暂时无法重启。请等待 PrintDeck 在线并完成当前操作。"],
  "PrintDeck could not accept the restart. Refresh its status and try again.":["PrintDeck nie przyjął żądania restartu. Odśwież stan urządzenia i spróbuj ponownie.", "PrintDeck no ha podido aceptar el reinicio. Actualiza su estado e inténtalo de nuevo.", "PrintDeck n’a pas pu accepter le redémarrage. Actualisez son état et réessayez.", "PrintDeck konnte den Neustart nicht annehmen. Aktualisiere den Status und versuche es erneut.", "PrintDeck 无法接受重启请求。请刷新状态后重试。"],

  "Hide the Live View bar?":["Ukryć belkę Live View?", "¿Ocultar la barra Live View?", "Masquer la barre Live View ?", "Live-View-Leiste ausblenden?", "隐藏实时画面栏？"],
  "Panda Knomi has no touchscreen. Live View is the only way to change its screens. If you hide the bar, you can still open Live View in Settings.":["Panda Knomi nie ma ekranu dotykowego. Live View to jedyny sposób zmiany ekranów. Po ukryciu belki nadal możesz otworzyć Live View w Ustawieniach.", "Panda Knomi no tiene pantalla táctil. Live View es la única forma de cambiar sus pantallas. Si ocultas la barra, podrás seguir abriendo Live View en Ajustes.", "Panda Knomi n’a pas d’écran tactile. Live View est le seul moyen de changer ses écrans. Si vous masquez la barre, vous pourrez toujours ouvrir Live View dans les paramètres.", "Panda Knomi hat keinen Touchscreen. Nur über Live View kannst du zwischen den Bildschirmen wechseln. Wenn du die Leiste ausblendest, kannst du Live View weiterhin in den Einstellungen öffnen.", "Panda Knomi 没有触摸屏，只能通过实时画面切换屏幕。隐藏此栏后，你仍可在设置中打开实时画面。"],
  "Hide bar":["Ukryj belkę", "Ocultar barra", "Masquer la barre", "Leiste ausblenden", "隐藏栏"],
  ...
  "Enter your computer’s local IP address and HTTP port for both the panel and API. Turn ON to save, or use Update while Developer Mode is on. Changing the address or port requires account pairing again.":["Wpisz lokalny adres IP komputera i port HTTP wspólny dla panelu i API. Włącz tryb, aby zapisać, lub użyj Aktualizuj, gdy tryb deweloperski jest włączony. Zmiana adresu lub portu wymaga ponownego parowania konta.", "Introduce la IP local del ordenador y el puerto HTTP compartido por el panel y la API. Activa el modo para guardar o usa Actualizar mientras esté activado. Cambiar la dirección o el puerto requiere vincular la cuenta de nuevo.", "Saisissez l’adresse IP locale de votre ordinateur et le port HTTP commun au panneau et à l’API. Activez le mode pour enregistrer ou utilisez Mettre à jour lorsqu’il est activé. Un changement d’adresse ou de port nécessite une nouvelle association du compte.", "Gib die lokale IP-Adresse deines Computers und den gemeinsamen HTTP-Port für Panel und API ein. Schalte den Modus zum Speichern ein oder nutze Aktualisieren bei aktivem Entwicklermodus. Nach einer Änderung der Adresse oder des Ports muss das Konto erneut verknüpft werden.", "输入电脑的本地 IP 地址以及面板和 API 共用的 HTTP 端口。开启模式以保存，或在开发者模式开启时点击更新。更改地址或端口后需要重新关联账户。"],
  ...
  "Access expires 30 minutes after unlocking.":["Dostęp wygasa po 30 minutach od odblokowania.", "El acceso caduca 30 minutos después del desbloqueo.", "L’accès expire 30 minutes après le déverrouillage.", "Der Zugriff endet 30 Minuten nach dem Entsperren.", "访问权限将在解锁 30 分钟后失效。"],
  "Enter the password to open Cloud settings.":["Wpisz hasło, aby otworzyć ustawienia Cloud.", "Introduce la contraseña para abrir los ajustes de Cloud.", "Saisissez le mot de passe pour ouvrir les paramètres Cloud.", "Gib das Passwort ein, um die Cloud-Einstellungen zu öffnen.", "输入密码以打开云设置。"],
  "DEVELOPER MODE":["TRYB DEWELOPERSKI", "MODO DESARROLLADOR", "MODE DÉVELOPPEUR", "ENTWICKLERMODUS", "开发者模式"],
  "ON sends printer reports and checks for commands on the local server below. OFF uses PrintDeck Cloud. Each environment has its own account pairing.":["ON wysyła raporty drukarek i sprawdza polecenia na poniższym serwerze lokalnym. OFF używa PrintDeck Cloud. Każde środowisko ma osobne parowanie z kontem.", "ON envía informes de impresoras y consulta comandos en el servidor local indicado abajo. OFF usa PrintDeck Cloud. Cada entorno tiene su propia vinculación de cuenta.", "ON envoie les rapports des imprimantes et consulte les commandes sur le serveur local ci-dessous. OFF utilise PrintDeck Cloud. Chaque environnement possède sa propre association de compte.", "ON sendet Druckerberichte und fragt Befehle beim unten angegebenen lokalen Server ab. OFF verwendet PrintDeck Cloud. Jede Umgebung hat ihre eigene Kontoverknüpfung.", "ON 将打印机报告发送到下方的本地服务器并查询命令。OFF 使用 PrintDeck Cloud。每个环境都需要单独关联账户。"],
  "Local API address":["Adres lokalnego API", "Dirección de la API local", "Adresse de l’API locale", "Lokale API-Adresse", "本地 API 地址"],
  "Keep the local server running and connect PrintDeck to the same network.":["Pozostaw serwer lokalny uruchomiony i połącz PrintDeck z tą samą siecią.", "Mantén el servidor local en funcionamiento y conecta PrintDeck a la misma red.", "Laissez le serveur local en marche et connectez PrintDeck au même réseau.", "Lass den lokalen Server laufen und verbinde PrintDeck mit demselben Netzwerk.", "请保持本地服务器运行，并将 PrintDeck 连接到同一网络。"],
  ...
  "PrintDeck Cloud":["PrintDeck Cloud", "PrintDeck Cloud", "PrintDeck Cloud", "PrintDeck Cloud", "PrintDeck Cloud"],
  "Connect this PrintDeck to your account. Sign in securely on the cloud website.":["Połącz ten PrintDeck ze swoim kontem. Zaloguj się bezpiecznie na stronie clouda.", "Conecta este PrintDeck a tu cuenta. Inicia sesión de forma segura en la web de la nube.", "Connectez ce PrintDeck à votre compte. Connectez-vous en toute sécurité sur le site cloud.", "Verbinde diesen PrintDeck mit deinem Konto. Melde dich sicher auf der Cloud-Website an.", "将此 PrintDeck 连接到你的账户。在云端网站安全登录。"],
  "PrintDeck shares printer status and supports available remote controls with the connected account.":["PrintDeck udostępnia stan drukarek i obsługuje dostępne zdalne polecenia dla połączonego konta.", "PrintDeck comparte el estado de las impresoras y permite los controles remotos disponibles con la cuenta vinculada.", "PrintDeck partage l’état des imprimantes et permet les commandes à distance disponibles avec le compte associé.", "PrintDeck teilt den Druckerstatus und unterstützt verfügbare Fernsteuerungen für das verknüpfte Konto.", "PrintDeck 与已关联账户共享打印机状态，并支持可用的远程控制。"],
  "Check that this code matches the code shown in the cloud:":["Sprawdź, czy ten kod jest zgodny z kodem pokazanym w cloudzie:", "Comprueba que este código coincide con el mostrado en la nube:", "Vérifiez que ce code correspond à celui affiché dans le cloud :", "Prüfe, ob dieser Code mit dem in der Cloud angezeigten Code übereinstimmt:", "请确认此代码与云端显示的代码一致："],
  "Connect account":["Połącz konto", "Conectar cuenta", "Connecter le compte", "Konto verbinden", "连接账户"],
  "Open PrintDeck Cloud":["Otwórz PrintDeck Cloud", "Abrir PrintDeck Cloud", "Ouvrir PrintDeck Cloud", "PrintDeck Cloud öffnen", "打开 PrintDeck Cloud"],
  "Disconnect account":["Odłącz konto", "Desconectar cuenta", "Déconnecter le compte", "Konto trennen", "断开账户连接"],
  "No cloud account connected.":["Brak połączonego konta cloud.", "No hay ninguna cuenta de nube conectada.", "Aucun compte cloud connecté.", "Kein Cloud-Konto verbunden.", "未连接云端账户。"],
  "Starting pairing…":["Rozpoczynanie parowania…", "Iniciando vinculación…", "Démarrage de l’association…", "Kopplung wird gestartet…", "正在开始配对…"],
  "Approve the connection in PrintDeck Cloud.":["Zatwierdź połączenie w PrintDeck Cloud.", "Aprueba la conexión en PrintDeck Cloud.", "Approuvez la connexion dans PrintDeck Cloud.", "Bestätige die Verbindung in PrintDeck Cloud.", "请在 PrintDeck Cloud 中批准连接。"],
  "Checking cloud connection…":["Sprawdzanie połączenia z cloudem…", "Comprobando la conexión con la nube…", "Vérification de la connexion au cloud…", "Cloud-Verbindung wird geprüft…", "正在检查云端连接…"],
  "Connected to PrintDeck Cloud.":["Połączono z PrintDeck Cloud.", "Conectado a PrintDeck Cloud.", "Connecté à PrintDeck Cloud.", "Mit PrintDeck Cloud verbunden.", "已连接到 PrintDeck Cloud。"],
  "Cloud is unavailable. PrintDeck will retry automatically.":["Cloud jest niedostępny. PrintDeck automatycznie ponowi próbę.", "La nube no está disponible. PrintDeck volverá a intentarlo automáticamente.", "Le cloud est indisponible. PrintDeck réessaiera automatiquement.", "Die Cloud ist nicht erreichbar. PrintDeck versucht es automatisch erneut.", "云端不可用。PrintDeck 将自动重试。"],
  "Cloud access was revoked. Connect your account again.":["Dostęp do clouda został unieważniony. Połącz konto ponownie.", "Se ha revocado el acceso a la nube. Vuelve a conectar tu cuenta.", "L’accès au cloud a été révoqué. Reconnectez votre compte.", "Der Cloud-Zugriff wurde widerrufen. Verbinde dein Konto erneut.", "云端访问权限已被撤销。请重新连接账户。"],
  "Pairing expired. Start again.":["Parowanie wygasło. Rozpocznij ponownie.", "La vinculación ha caducado. Vuelve a empezar.", "L’association a expiré. Recommencez.", "Die Kopplung ist abgelaufen. Starte erneut.", "配对已过期。请重新开始。"],
  "Pairing was declined.":["Parowanie zostało odrzucone.", "Se ha rechazado la vinculación.", "L’association a été refusée.", "Die Kopplung wurde abgelehnt.", "配对已被拒绝。"],
  "Disconnecting… Waiting for cloud confirmation.":["Odłączanie… Oczekiwanie na potwierdzenie clouda.", "Desconectando… Esperando confirmación de la nube.", "Déconnexion… En attente de confirmation du cloud.", "Verbindung wird getrennt… Warte auf die Cloud-Bestätigung.", "正在断开连接… 等待云端确认。"],
  "Cloud request failed. Try again.":["Operacja cloud nie powiodła się. Spróbuj ponownie.", "La solicitud a la nube ha fallado. Vuelve a intentarlo.", "La demande au cloud a échoué. Réessayez.", "Die Cloud-Anfrage ist fehlgeschlagen. Versuche es erneut.", "云端请求失败。请重试。"],

  "Save broker settings":["Zapisz ustawienia brokera", "Guardar ajustes del servidor", "Enregistrer les paramètres du serveur", "Broker-Einstellungen speichern", "保存服务器设置"],
  "Set up your broker":["Skonfiguruj brokera", "Configura tu servidor", "Configurez votre serveur", "Broker einrichten", "配置服务器"],
  "Enter your broker settings, then select Save & connect to enable MQTT.":["Podaj ustawienia brokera, a następnie wybierz Zapisz i połącz, aby włączyć MQTT.", "Introduce los ajustes del servidor y selecciona Guardar y conectar para activar MQTT.", "Saisissez les paramètres du serveur, puis sélectionnez Enregistrer et connecter pour activer MQTT.", "Trage die Broker-Einstellungen ein und wähle Speichern und verbinden, um MQTT zu aktivieren.", "输入服务器设置，然后选择“保存并连接”以启用 MQTT。"],

  "REST API":["REST API", "API REST", "API REST", "REST-API", "REST API"],
  "Share printer data with local applications over HTTP. Access requires an API token.":["Udostępnia dane drukarek lokalnym aplikacjom przez HTTP. Dostęp wymaga tokenu API.", "Comparte datos de las impresoras con aplicaciones locales mediante HTTP. El acceso requiere un token de API.", "Partage les données des imprimantes avec les applications locales via HTTP. L’accès nécessite un jeton API.", "Teilt Druckerdaten über HTTP mit lokalen Anwendungen. Der Zugriff erfordert ein API-Token.", "通过 HTTP 与本地应用共享打印机数据。访问需要 API 令牌。"],

  "Turn off this connection?":["Wyłączyć to połączenie?", "¿Desactivar esta conexión?", "Désactiver cette connexion ?", "Diese Verbindung ausschalten?", "要关闭此连接吗？"],
  "Turn off":["Wyłącz", "Desactivar", "Désactiver", "Ausschalten", "关闭"],
  "Unsaved changes":["Niezapisane zmiany", "Cambios sin guardar", "Modifications non enregistrées", "Ungespeicherte Änderungen", "未保存的更改"],
  "HTTP API access will stop immediately. Applications and Home Assistant integrations using this API will stop receiving data. Your token will be kept for when you turn it on again.":["Dostęp do API HTTP zostanie natychmiast wyłączony. Aplikacje i integracje Home Assistant korzystające z tego API przestaną otrzymywać dane. Token zostanie zachowany do ponownego włączenia.", "El acceso a la API HTTP se detendrá inmediatamente. Las aplicaciones e integraciones de Home Assistant que usan esta API dejarán de recibir datos. El token se conservará para cuando vuelvas a activarla.", "L’accès à l’API HTTP sera immédiatement désactivé. Les applications et intégrations Home Assistant utilisant cette API ne recevront plus de données. Votre jeton sera conservé pour la prochaine activation.", "Der HTTP-API-Zugriff wird sofort beendet. Anwendungen und Home-Assistant-Integrationen, die diese API verwenden, erhalten keine Daten mehr. Dein Token bleibt für das erneute Einschalten gespeichert.", "HTTP API 访问将立即停止。使用此 API 的应用和 Home Assistant 集成将停止接收数据。令牌会保留，以便再次启用。"],
  "PrintDeck will stop sending MQTT data and print events. Home Assistant entities using MQTT will become unavailable or be removed during Discovery cleanup. Broker settings will be kept.":["PrintDeck przestanie wysyłać dane MQTT i zdarzenia druku. Encje Home Assistant korzystające z MQTT staną się niedostępne lub zostaną usunięte podczas porządkowania Discovery. Ustawienia brokera zostaną zachowane.", "PrintDeck dejará de enviar datos MQTT y eventos de impresión. Las entidades de Home Assistant que usan MQTT quedarán no disponibles o se eliminarán durante la limpieza de la detección. Se conservarán los ajustes del servidor.", "PrintDeck cessera d’envoyer les données MQTT et les événements d’impression. Les entités Home Assistant utilisant MQTT deviendront indisponibles ou seront supprimées pendant le nettoyage de la découverte. Les paramètres du serveur seront conservés.", "PrintDeck sendet keine MQTT-Daten und Druckereignisse mehr. MQTT-Entitäten in Home Assistant werden nicht verfügbar oder bei der Discovery-Bereinigung entfernt. Die Broker-Einstellungen bleiben gespeichert.", "PrintDeck 将停止发送 MQTT 数据和打印事件。Home Assistant 中使用 MQTT 的实体将不可用，或在自动发现清理期间被删除。服务器设置会保留。"],
  "PrintDeck will remove its automatically created MQTT entities from Home Assistant. Dashboards and automations using those entities may stop working. Keep MQTT connected until cleanup finishes.":["PrintDeck usunie z Home Assistant swoje automatycznie utworzone encje MQTT. Panele i automatyzacje korzystające z tych encji mogą przestać działać. Pozostaw MQTT połączone do zakończenia porządkowania.", "PrintDeck eliminará de Home Assistant sus entidades MQTT creadas automáticamente. Los paneles y automatizaciones que las usan pueden dejar de funcionar. Mantén MQTT conectado hasta que termine la limpieza.", "PrintDeck supprimera de Home Assistant ses entités MQTT créées automatiquement. Les tableaux de bord et automatisations qui les utilisent risquent de ne plus fonctionner. Gardez MQTT connecté jusqu’à la fin du nettoyage.", "PrintDeck entfernt seine automatisch erstellten MQTT-Entitäten aus Home Assistant. Dashboards und Automationen mit diesen Entitäten funktionieren möglicherweise nicht mehr. Lass MQTT bis zum Abschluss der Bereinigung verbunden.", "PrintDeck 将从 Home Assistant 中删除其自动创建的 MQTT 实体。使用这些实体的仪表盘和自动化可能停止工作。请保持 MQTT 连接，直到清理完成。"],

  "Your password is saved. Leave this field empty to keep it, or enter a new password to replace it.":["Hasło jest zapisane. Pozostaw pole puste, aby je zachować, lub wpisz nowe hasło, aby je zastąpić.", "Tu contraseña está guardada. Deja este campo vacío para conservarla o introduce una nueva para sustituirla.", "Votre mot de passe est enregistré. Laissez ce champ vide pour le conserver ou saisissez un nouveau mot de passe pour le remplacer.", "Dein Passwort ist gespeichert. Lass dieses Feld leer, um es zu behalten, oder gib ein neues Passwort ein, um es zu ersetzen.", "密码已保存。留空可保留原密码，输入新密码可替换原密码。"],
  "If your broker requires a password, enter it here.":["Jeśli broker wymaga hasła, wpisz je tutaj.", "Si tu servidor requiere una contraseña, introdúcela aquí.", "Si votre serveur exige un mot de passe, saisissez-le ici.", "Falls dein Broker ein Passwort benötigt, gib es hier ein.", "如果服务器需要密码，请在此输入。"],
  "Your CA certificate is saved. Leave this field empty to keep it, or paste a new certificate to replace it.":["Certyfikat CA jest zapisany. Pozostaw pole puste, aby go zachować, lub wklej nowy certyfikat, aby go zastąpić.", "Tu certificado CA está guardado. Deja este campo vacío para conservarlo o pega uno nuevo para sustituirlo.", "Votre certificat CA est enregistré. Laissez ce champ vide pour le conserver ou collez un nouveau certificat pour le remplacer.", "Dein CA-Zertifikat ist gespeichert. Lass dieses Feld leer, um es zu behalten, oder füge ein neues Zertifikat ein, um es zu ersetzen.", "CA 证书已保存。留空可保留原证书，粘贴新证书可替换原证书。"],
  "Paste a CA certificate only if your broker uses a private certificate authority.":["Wklej certyfikat CA tylko wtedy, gdy broker korzysta z prywatnego urzędu certyfikacji.", "Pega un certificado CA solo si tu servidor utiliza una autoridad de certificación privada.", "Collez un certificat CA uniquement si votre serveur utilise une autorité de certification privée.", "Füge nur dann ein CA-Zertifikat ein, wenn dein Broker eine private Zertifizierungsstelle verwendet.", "仅当服务器使用私有证书颁发机构时，才需粘贴 CA 证书。"],

  "Enable MQTT to make this PrintDeck available to the PrintDeck integration in Home Assistant. HTTP access remains independent.":["Włącz MQTT, aby ten PrintDeck był dostępny do dodania w integracji PrintDeck w Home Assistant. Dostęp HTTP działa niezależnie.", "Activa MQTT para que este PrintDeck se pueda añadir a la integración PrintDeck de Home Assistant. El acceso HTTP es independiente.", "Activez MQTT pour pouvoir ajouter ce PrintDeck à l’intégration PrintDeck dans Home Assistant. L’accès HTTP reste indépendant.", "Aktiviere MQTT, damit dieser PrintDeck in der PrintDeck-Integration von Home Assistant hinzugefügt werden kann. Der HTTP-Zugriff bleibt unabhängig.", "启用 MQTT 后，即可在 Home Assistant 的 PrintDeck 集成中添加此设备。HTTP 访问独立运行。"],
  "For Home Assistant’s built-in MQTT integration. Keep OFF when using the PrintDeck HACS integration.":["Dla wbudowanej integracji MQTT w Home Assistant. Pozostaw wyłączone, jeśli używasz integracji PrintDeck z HACS.", "Para la integración MQTT integrada en Home Assistant. Mantén desactivado si usas la integración PrintDeck de HACS.", "Pour l’intégration MQTT native de Home Assistant. Laissez désactivé avec l’intégration PrintDeck de HACS.", "Für die integrierte MQTT-Integration von Home Assistant. Bei der PrintDeck-Integration aus HACS ausgeschaltet lassen.", "用于 Home Assistant 内置 MQTT 集成。使用 HACS 中的 PrintDeck 集成时，请保持关闭。"],
  "Turn MQTT on above to use Discovery.":["Aby korzystać z Discovery, włącz MQTT powyżej.", "Activa MQTT arriba para usar la detección.", "Activez MQTT ci-dessus pour utiliser la découverte.", "Aktiviere oben MQTT, um Discovery zu verwenden.", "请先在上方启用 MQTT，才能使用自动发现。"],
  "Home Assistant creates MQTT entities automatically. Before switching to HACS, turn Discovery off and wait for the old entities to disappear. Keep MQTT connected during cleanup.":["Home Assistant automatycznie tworzy encje MQTT. Przed przejściem na HACS wyłącz Discovery i poczekaj, aż stare encje znikną. Podczas porządkowania pozostaw MQTT połączone.", "Home Assistant crea entidades MQTT automáticamente. Antes de cambiar a HACS, desactiva la detección y espera a que desaparezcan las entidades anteriores. Mantén MQTT conectado durante la limpieza.", "Home Assistant crée automatiquement les entités MQTT. Avant de passer à HACS, désactivez la découverte et attendez la disparition des anciennes entités. Gardez MQTT connecté pendant le nettoyage.", "Home Assistant erstellt MQTT-Entitäten automatisch. Schalte vor dem Wechsel zu HACS Discovery aus und warte, bis die alten Entitäten verschwinden. Lass MQTT während der Bereinigung verbunden.", "Home Assistant 会自动创建 MQTT 实体。切换到 HACS 前，请关闭自动发现并等待旧实体消失。清理期间请保持 MQTT 连接。"],
  ...
  "Check card":["Sprawdź kartę", "Comprobar tarjeta", "Vérifier la carte", "Karte prüfen", "检查存储卡"],
  ...
  "PrintDeck could not save these changes. Please try again.":["PrintDeck nie mógł zapisać tych zmian. Spróbuj ponownie.","PrintDeck no ha podido guardar estos cambios. Inténtalo de nuevo.","PrintDeck n’a pas pu enregistrer ces modifications. Réessayez.","PrintDeck konnte diese Änderungen nicht speichern. Versuche es erneut.","PrintDeck 无法保存这些更改。请重试。"],
  ...
  "PrintDeck is already checking or monitoring this printer. Try again shortly.":["PrintDeck już sprawdza lub monitoruje tę drukarkę. Spróbuj ponownie za chwilę.","PrintDeck ya está comprobando o supervisando esta impresora. Inténtalo de nuevo en un momento.","PrintDeck vérifie ou surveille déjà cette imprimante. Réessayez dans un instant.","PrintDeck prüft oder überwacht diesen Drucker bereits. Versuche es gleich erneut.","PrintDeck 已在检查或监控此打印机。请稍后重试。"],
  ...
  "Refresh every 10 seconds":["Odświeżaj co 10 sekund","Actualizar cada 10 segundos","Actualiser toutes les 10 secondes","Alle 10 Sekunden aktualisieren","每 10 秒刷新"],
  "Choose a supported printer refresh interval.":["Wybierz obsługiwany interwał odświeżania drukarek.","Elige un intervalo de actualización de impresoras admitido.","Choisissez un intervalle d’actualisation des imprimantes pris en charge.","Wähle ein unterstütztes Aktualisierungsintervall für Drucker.","请选择支持的打印机刷新间隔。"],
  ...
  ,"Print failed":["Błąd wydruku","Error de impresión","Échec de l’impression","Druck fehlgeschlagen","打印失败"]
  ,"Last check: {state}":["Ostatni odczyt: {state}","Última consulta: {state}","Dernier relevé : {state}","Letzte Abfrage: {state}","上次读取：{state}"]
  ...
  ,"Choose a notification sound style and volume. Every preset uses the same PrintDeck startup tune.":["Wybierz styl i głośność powiadomień. Wszystkie zestawy używają tej samej melodii startowej PrintDeck.","Elige el estilo y el volumen de las notificaciones. Todos los estilos usan la misma melodía de inicio de PrintDeck.","Choisissez le style et le volume des notifications. Tous les styles utilisent la même mélodie de démarrage PrintDeck.","Wähle Klangstil und Lautstärke für Benachrichtigungen. Alle Klangstile verwenden dieselbe PrintDeck-Startmelodie.","选择通知的声音风格和音量。所有预设都使用相同的 PrintDeck 启动旋律。"]
  ...
  ,"Use a password with at least 3 characters.":["Użyj hasła składającego się z co najmniej 3 znaków.","Usa una contraseña de al menos 3 caracteres.","Utilisez un mot de passe d’au moins 3 caractères.","Verwende ein Passwort mit mindestens 3 Zeichen.","请使用至少 3 个字符的密码。"]
...
// Resin printer setup, in the maintained language order.
const RESIN_SETUP_TRANSLATIONS={
  "RESIN": [
    "ŻYWICZNA",
    "RESINA",
    "RÉSINE",
  ...
  "UniFormation LAN": [
    "UniFormation LAN",
    "LAN de UniFormation",
    "Réseau local UniFormation",
    "UniFormation LAN",
    "UniFormation 局域网"
  ],
  "Add UniFormation printer": [
    "Dodaj drukarkę UniFormation",
    "Añadir impresora UniFormation",
    "Ajouter une imprimante UniFormation",
    "UniFormation-Drucker hinzufügen",
    "添加 UniFormation 打印机"
  ],
  "PrintDeck checks new or changed connections. No printer password is needed.": ["PrintDeck sprawdza nowe lub zmienione połączenia. Hasło drukarki nie jest potrzebne.", "PrintDeck comprueba las conexiones nuevas o modificadas. No se necesita contraseña de impresora.", "PrintDeck vérifie les connexions nouvelles ou modifiées. Aucun mot de passe d’imprimante n’est nécessaire.", "PrintDeck prüft neue oder geänderte Verbindungen. Ein Druckerpasswort ist nicht erforderlich.", "PrintDeck 会检查新增或更改的连接。无需打印机密码。"],
  "Select this printer to view its resin print status on PrintDeck.": ["Wybierz tę drukarkę, aby zobaczyć na PrintDeck status wydruku żywicznego.", "Selecciona esta impresora para ver el estado de impresión en resina en PrintDeck.", "Sélectionnez cette imprimante pour afficher son impression résine sur PrintDeck.", "Wähle diesen Drucker, um seinen Resin-Druckstatus auf PrintDeck zu sehen.", "选择此打印机，即可在 PrintDeck 上查看树脂打印状态。"],
  "The device view for this printer is not available yet.": [
    "Widok tej drukarki na urządzeniu nie jest jeszcze dostępny.",
    "La vista de esta impresora en el dispositivo aún no está disponible.",
    "La vue de cette imprimante sur l’appareil n’est pas encore disponible.",
    "Die Geräteansicht für diesen Drucker ist noch nicht verfügbar.",
    "设备上暂时无法显示此打印机的详情。"
  ],
  "Check the UniFormation connection before saving.": [
    "Sprawdź połączenie z UniFormation przed zapisaniem.",
    "Comprueba la conexión con UniFormation antes de guardar.",
    "Vérifiez la connexion UniFormation avant d’enregistrer.",
    "Prüfe vor dem Speichern die Verbindung zu UniFormation.",
    "请先检查 UniFormation 连接，再保存。"
  ],
  "UniFormation did not respond as a supported GK3 Ultra. Check the address and local network.": [
    "Nie udało się potwierdzić zgodnej drukarki UniFormation GK3 Ultra. Sprawdź adres i sieć lokalną.",
    "No se pudo confirmar una UniFormation GK3 Ultra compatible. Comprueba la dirección y la red local.",
    "Impossible de confirmer une UniFormation GK3 Ultra compatible. Vérifiez l’adresse et le réseau local.",
    "Ein unterstützter UniFormation GK3 Ultra konnte nicht bestätigt werden. Prüfe Adresse und lokales Netzwerk.",
    "未能确认支持的 UniFormation GK3 Ultra 打印机。请检查地址和局域网。"
  ]
};
...
// Native camera viewing preferences.
[["Cameras", "Kamery", "Cámaras", "Caméras", "Kameras", "摄像头"]].forEach(([source,...values])=>PRINTDECK_TRANSLATION_COLUMNS.forEach((language,index)=>window.PRINTDECK_TRANSLATIONS[language][source]=values[index]));
...
[["Device Live View", "Live View urządzenia", "Live View del dispositivo", "Live View de l’appareil", "Geräte-Live-View", "设备 Live View"], ["Close device screen", "Zamknij ekran urządzenia", "Cerrar la pantalla del dispositivo", "Fermer l’écran de l’appareil", "Gerätebildschirm schließen", "关闭设备屏幕"], ["Quick Menu", "Szybkie menu", "Menú rápido", "Menu rapide", "Schnellmenü", "快捷菜单"]].forEach(([source,...values])=>PRINTDECK_TRANSLATION_COLUMNS.forEach((language,index)=>window.PRINTDECK_TRANSLATIONS[language][source]=values[index]));

[["Show Live View bar", "Pokaż pasek Live View", "Mostrar la barra de Live View", "Afficher la barre Live View", "Live-View-Leiste anzeigen", "显示 Live View 栏"], ["Display Device Live View bar on every page.", "Wyświetlaj pasek Live View urządzenia na każdej stronie.", "Muestra la barra de Live View del dispositivo en todas las páginas.", "Affiche la barre Live View de l’appareil sur chaque page.", "Zeige die Live-View-Leiste des Geräts auf jeder Seite an.", "在每个页面显示设备 Live View 栏。"]].forEach(([source,...values])=>PRINTDECK_TRANSLATION_COLUMNS.forEach((language,index)=>window.PRINTDECK_TRANSLATIONS[language][source]=values[index]));
